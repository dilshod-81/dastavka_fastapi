...
@order_router.get('/')
async def welcome_page(Authorize: AuthJWT=Depends()): # eski nomi order
    try:
...
async def make_order(order: OrderModel, Authorize: AuthJWT=Depends()):
    ...
    data = {
        ...
        "data": {
            ...
            "order_statuses": new_order.order_statuses.value, # yangilandi
...
async def update_order(id:int, order: OrderModel, Authorize: AuthJWT=Depends()):
    ...
    if order_to_update.user != user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Kechirasiz, siz boshqa userning buyurtmalarini tahrirlay olmaysiz!")
...
async def update_order_status(id: int, order: OrderStatusModel, Authorize: AuthJWT=Depends()):
    ...
    if user.is_staff:
        ...
        custom_response = {
            "success": True,
            "code": 200,
            "message": "User orders is successful updated",
            "data": {
...
async def delete_order(id: int,  Authorize: AuthJWT=Depends()):
    ...
    if order.user != user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Kechirasiz, siz boshqa userning buyurtmalarini o'chira olmaysiz!")
    if order.order_statuses != "PENDING":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Kechirasiz, siz yo'lga chiqqan va yetkazib berilgan buyurtmalarni o'chira olmaysiz!")
    session.delete(order)
    session.commit()
    custom_response = {
        "success": True,
        "code": 200,
        "message": "User orders is successful deleted",
        "data": None
    }
    return jsonable_encoder(custom_response)
